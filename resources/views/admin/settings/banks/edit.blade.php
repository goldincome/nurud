...
@section('content')
    <div class="max-w-3xl mx-auto">
        <div class="flex items-center space-x-2 mb-6 text-sm text-slate-500">
            ...
            <span>/</span>
            <a href="{{ route('admin.banks.index') }}" class="hover:text-brand-blue transition">{{ $bank->bank_name }}</a>
            <span>/</span>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50">
                <h2 class="text-lg font-bold text-slate-800">Edit Bank Account</h2>
                <p class="text-xs text-slate-500 mt-1">Update bank account details.</p>
            </div>

            <form action="{{ route('admin.banks.update', $bank->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1">
                        ...
                        <select name="country_id" id="country_id"
                            ...
                            @foreach($countries as $country)
                                <option value="{{ $country->id }}" {{ old('country_id', $bank->country_id) == $country->id ? 'selected' : '' }}>
                    ...
                    <div class="space-y-1">
                        <label for="deadline" class="text-sm font-medium text-slate-700">Payment Deadline (Hours) <span
                                class="text-red-500">*</span></label>
                        <input type="number" name="deadline" id="deadline" value="{{ old('deadline', $bank->deadline) }}"
                            class="w-full rounded-lg border-slate-300 focus:border-brand-blue focus:ring-brand-blue/20 transition-colors placeholder-slate-400 text-sm"
                            placeholder="e.g. 24">
                    ...
                    <div class="space-y-1 md:col-span-2">
                        ...
                        <textarea name="instructions" id="instructions" rows="4"
                            class="w-full rounded-lg border-slate-300 focus:border-brand-blue focus:ring-brand-blue/20 transition-colors placeholder-slate-400 text-sm"
                            placeholder="Enter any specific payment instructions or notes for the customer...">{{ old('instructions', $bank->instructions) }}</textarea>
                        @error('instructions')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
